...
| 10 Edge cases | ✅ | 2026-09-07 | Verified the untestable-without-a-browser items myself: real 34MB file rejected cleanly, a genuinely zero-text-layer scanned PDF read correctly via Claude's vision (validates §8.1's whole-PDF architecture decision). seedData.js finally removed — app now genuinely starts blank. Rest confirmed by hand in the browser. |
| 11 Deploy | ✅ | 2026-09-07 | Live at curriculum-creation-engine.vercel.app. Verified the real AI route end-to-end in production, not just the static page — same test PDF as local testing, correctly detected "structured" with 3 modules, ~18s, no timeout. Confirms ANTHROPIC_API_KEY and maxDuration are both correctly configured on Vercel. |
| 12 Video | ⬜ | | |
